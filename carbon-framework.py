...
Notes:
----------
For more detailed, annotated versions of this code, see the Jupyter notebooks
in the `notebooks/` directory of this repository.
...
# ---------------------------------------------------------------------------
# Per-node power readings are fetched from the CSD3 VictoriaMetrics time-series
# database, which is only reachable from within the Cambridge Research Computing
# network. The original endpoint, proxy, and authentication details are NOT
# included in this public version of the script.
#
# To run the power and energy calculations, provide your own access details via
# environment variables:
#
#   VICTORIA_METRICS_URL    URL of the VictoriaMetrics query endpoint   (required)
#   VICTORIA_METRICS_PROXY  proxy URL, if one is needed to reach it     (optional)
#   VICTORIA_METRICS_TOKEN  bearer token sent as an Authorization header(optional)
#
# getJobPower() issues:
#   requests.put(url, data=<PromQL query>, proxies=proxies, headers=headers)
# selecting the `amperageProbeReading` metric for the job's nodes.
#
# If VICTORIA_METRICS_URL is unset, getJobPower() raises a clear RuntimeError
# rather than failing with a NameError.
# ---------------------------------------------------------------------------

url = os.environ.get("VICTORIA_METRICS_URL")
proxies = (
    {"http": os.environ["VICTORIA_METRICS_PROXY"],
     "https": os.environ["VICTORIA_METRICS_PROXY"]}
    if os.environ.get("VICTORIA_METRICS_PROXY") else None
)
headers = (
    {"Authorization": f"Bearer {os.environ['VICTORIA_METRICS_TOKEN']}"}
    if os.environ.get("VICTORIA_METRICS_TOKEN") else None
)


def _require_victoria_metrics():
    """Raise a clear error if VictoriaMetrics access has not been configured.

    Per-node power data is only available from inside the Cambridge Research
    Computing network, so this script cannot fetch it in a standalone or public
    environment without the caller supplying their own endpoint details.
    """
    if not url:
        raise RuntimeError(
            "VictoriaMetrics access is not configured. carbon-framework.py fetches "
            "per-node power readings from the CSD3 VictoriaMetrics endpoint, which is "
            "only reachable from within the Cambridge Research Computing network. Set "
            "the VICTORIA_METRICS_URL environment variable (and, if required, "
            "VICTORIA_METRICS_PROXY and VICTORIA_METRICS_TOKEN) to run the power and "
            "energy calculations."
        )
